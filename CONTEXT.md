...
- **Bugs cazados con capturas:** `cleanup` fuera del `try` no veía `onResize` (pantalla negra al abrir carta) → handlers en `let` externos con guardas. Móvil vertical quedaba dentro de la mesa → FOV 58 + distancia follow ×2.2 en portrait. `forceContextLoss` en cleanup envenenaba la re-creación inmediata (la app se remonta sola al inicio + StrictMode monta doble) → solo `dispose()`. `getTangentAt(lapT)` sin envolver rompía desde la vuelta 2 (`distanceToSquared` de undefined) → todo muestreo con `lapT % 1`.
