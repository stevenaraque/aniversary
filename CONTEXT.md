...
- **Cómo (sin dañar):** `src/components/origamiButterfly.js` exporta `mountOrigamiButterfly(canvas, {size})`; `Letter.jsx` lo importa con `import()` dinámico SOLO en `phase==='flying'` → three queda en chunk separado (`origamiButterfly-*.js` ~476kB), el bundle inicial no crece (sigue ~489kB). Canvas 112×112 transparente montado en la caja que ya movía el vuelo (trayectoria, tiempos, sparkles, trails y carta intactos). Cleanup total al salir de fase/unmount + pausa en pestaña oculta + aleteo lento (0.6Hz, amp 0.45) con `prefers-reduced-motion` en vez de congelar.
- **Ajustes visuales:** cámara 3/4 desde arriba (de frente solo se veía el morro), `emissive` suave (el washi se diseñó sobre crema y se apagaba en obsidian), escala 1.15.
